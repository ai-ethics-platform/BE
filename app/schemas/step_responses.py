...
class DilemmaResponse(BaseModel):
    ...
    # dilemma → flip: question, choice1, choice2 필요
...
class FlipResponse(BaseModel):
    ...
    # flip → roles: structure 필요


class RolesResponse(BaseModel):
    """roles 단계 응답 모델 - structure, role 변수 추출 (ending 단계에 전달)"""
    response_text: str = Field(..., description="사용자에게 보여줄 응답 텍스트")
    structure: Optional[str] = Field(None, description="다음 단계(ending)에 전달할 structure 변수")
    role: Optional[str] = Field(None, description="다음 단계(ending)에 전달할 role 변수")
    # roles → ending: structure, role 필요


class EndingResponse(BaseModel):
    """ending 단계 응답 모델 - 마지막 단계 (변수 불필요)"""
    response_text: str = Field(..., description="사용자에게 보여줄 응답 텍스트")
